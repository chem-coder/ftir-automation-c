// Part of T_seq macro, works with
// Eurotherm Temperature Controller
//
// Program function: 
// Sets the setpoint to 25C and adjusts PID parameters accordingly
//
// 9/7/12 DKM
...
// Calculate and send the appropriate Proportional band
xx_val = -0.00000001*PV*PV*PV + .00002*PV*PV - 0.011*PV + 4.3414;
...
printf (" Pb = %s", xx_strng);

// Calculate the appropriate Integral time ti and send
xx_val = .000004*PV*PV - 0.0026*PV + .9099;
sprintf (xx_strng, "%1.3f", xx_val);
...
sprintf (xx_strng, "%2.2f", PV);
sprintf (wt_xx, "SL%s%c", xx_strng, ETX);
BCC = mk_BCC_checksum(wt_xx);
sprintf (wt_xx, "%c0000%cSL%s%c%c", EOT, STX, xx_strng, ETX, BCC);
r_str = send_str(wt_xx);
printf ("\nSetpoint: %s", xx_strng);
